  ...
  return (
    <>
      ...
      <StreamsLineChart
        ...
        getLabel={(k) => albums.items.find((a) => a.album_uri === k)?.album_short_name}
        getCurrentRank={(k) => (albums.items.find((a) => a.album_uri === k)?.album_stream_count ?? 0) * -1}
        getImageURL={(k) => albums.items.find((a) => a.album_uri === k)?.album_image_url}
      />
    </>
  );
}
