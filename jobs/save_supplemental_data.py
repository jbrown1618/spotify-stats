...
def save_supplemental_data():
    ...
    with get_engine().begin() as conn:
        result = conn.execute(sqlalchemy.text(query_text('unfetched_isrcs')))
        unfetched = result.fetchall()

        if should_retry_unfetchable_data():
            result = conn.execute(sqlalchemy.text(query_text('missing_credits')))
            missing_credits = result.fetchall()
